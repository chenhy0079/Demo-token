...
public interface LoadBalancerMapper {
    ...
    @Select("SELECT loadBalancerId FROM " + GlobalConstant.LOADBALANCERDEPLOYMAP_TABLE_NAME + " WHERE deployId = #{deployId}")
    int selectLbByDeployId( @Param("deployId") int deployId);
    
    @Delete("delete from " + GlobalConstant.LOADBALANCERDEPLOYMAP_TABLE_NAME + " where deployId = #{deployId}")
    void deleteLbMpByDeployId( @Param("deployId") int deployId);
    
    @Delete("delete from " + GlobalConstant.LOADBALANCER_TABLE_NAME + " where id = #{id}")
    void deleteLbByDeployId( @Param("id") int id);
}
